...
impl Totals {
    pub fn new_from(tags: &HashMap<String, Tag>, sites: &HashMap<String, Site>) -> Self {
        ...
        let mut tag_ratios: Vec<(&String, f64)> = 
            tags
            .iter()
        ...
        let mut sites_ratios: Vec<(&String, f64)> = 
            sites
            .iter()
        ...
        // Ordenamos el vector de tuplas por la relación number_of_words/number_of_questions en orden descendente
        sites_ratios.sort_by(|(_, ratio1), (_, ratio2)| match ratio2.partial_cmp(ratio1) {
            Some(o) => o,
            None => Ordering::Equal,
        });

        // Tomamos los primeros 10 elementos del vector
        let top_10_sites: Vec<&String> = tag_ratios.iter().take(10).map(|(name, _)| *name).collect();
